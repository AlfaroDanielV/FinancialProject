...
- **Phase 6a: `split_for_telegram` is dormant defensive code.** `app/queries/llm_client._create_message` sets `max_tokens=700` (~2800 chars), so a single LLM response can't reach the splitter's `cap=3900`. The 14 unit tests in `tests/test_delivery_split.py` cover the multi-chunk path with synthetic input, but no real response has ever been split in production or e2e. **Before raising `max_tokens` to 1500/2000** (which you'll want when users start asking for exhaustive breakdowns), add an e2e test that produces a >3900-char real response and verifies each chunk parses as valid Telegram HTML. Otherwise the splitter goes from "dormant defense" to "hot path never exercised against real LLM output" in one commit.
